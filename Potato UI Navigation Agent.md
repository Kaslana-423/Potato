...
你正在维护一个 Unity 2022.3 项目。项目已有主菜单、角色开始页、商店和波次战斗的运行截图，但完整一局流程仍需 Play Mode 回归，不能仅根据截图宣称已经完整验收。
...
`Assets/ScreenPost` 当前记录了以下界面：

| 界面 | 截图 | 可确认内容 | 尚不能确认 |
| --- | --- | --- | --- |
| 主菜单 | `b91308c5cbc3006ed0aea68ad28671b2.png` | 纸张菜单包含 New Game、Continue、Settings、Stats、Exit，当前焦点有箭头提示 | 存档状态是否正确驱动 Continue、连续返回和键盘/鼠标焦点恢复 |
| 角色开始页 | `91c24d428502f121fc1d0290df5a9be9.png` | 角色插画、标题和开始提示已显示 | 角色/武器切换、返回层级、确认输入和存档上下文 |
| 商店 | `6418cced06aee525fb16f1c05aea9c61.png` | 属性栏、四张商品卡、刷新、图钉、背包、翻页和下一关入口已显示 | 拖动购买、锁定刷新、合成/回收、下一波衔接和保存恢复 |
| 波次战斗 | `061db0dc30de9327f7366ee44a8beef6.png` | 第 2 波 HUD、生命、经验、材料、倒计时、攻击和伤害数字已显示 | 暂停子树、死亡/胜利结算、波后奖励及完整 20 波流程 |

当前视觉确实采用纸张、手绘线稿和暖色调，但本文档的导航设计仍只讨论信息架构与交互规则。截图中的 `keke!`、`Ghost Axe`、`Ghost Flint` 表明语言尚未统一；战斗画面右侧红色方块也应作为待排查的占位表现，而不是完成态视觉。
